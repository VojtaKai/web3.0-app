...
const Transaction = (props: TransactionProps) => {
    const {url, addressFrom, addressTo, amount, timestamp} = props
    return (
        <li className='md:3/12 m-2'>
            <div className='flex flex-col bg-blue-600 rounded-3xl md:4/12 p-2'>
                ...
                <h1>{`Amount: ${amount}`}</h1>
                <img src={url} className='px-2 rounded-xl w-96 h-96 self-center'></img>
    ...
    return (
        <div className='flex flex-col items-center py-10'>
            ...
            <ul className='flex flex-row justify-center flex-wrap'>
                {dummyData.reverse().map((transaction: TransactionProps) => <Transaction {...transaction}/>)}
            </ul>
        </div>
    )
}